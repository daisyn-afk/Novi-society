...
async function runWebhookSideEffect(label, fn) {
  try {
    await fn();
  } catch (error) {
    // Stripe retries on non-2xx; business-handler failures are logged and reconciled separately.
    // eslint-disable-next-line no-console
    console.error(`[webhook] ${label} failed:`, error?.message || error);
  }
}
...
webhooksRouter.post("/stripe-connect", async (req, res, next) => {
  try {
    ...
    await runWebhookSideEffect(`stripe-connect ${event.type} ${event.id}`, () =>
      processStripeConnectWebhookEvent(event)
    );
    // eslint-disable-next-line no-console
    console.log("[webhook/stripe-connect] acknowledged", event.type, event.id);
...
webhooksRouter.post("/stripe", async (req, res) => {
  try {
    ...
    await runWebhookSideEffect(`stripe tracker ${event.type} ${event.id}`, async () => {
      if (TRACKED_STRIPE_EVENT_TYPES.has(event.type)) {
        await recordStripeWebhookEvent(event);
      }
    });

    await runWebhookSideEffect(`stripe business ${event.type} ${event.id}`, async () => {
      ...
      if (
        ...
        if (!patientResult?.skipped) return;
        await processMdBoardStripeEvent(event);
        return;
      }
    ...
    // eslint-disable-next-line no-console
    console.log("[webhook/stripe] acknowledged", event.type, event.id);
    return res.json({ received: true });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error("[webhook/stripe] unexpected error:", error?.message || error);
