...
export async function uploadPatientJourneySelfie({ buffer, mimeType, extension, patientId }) {
  const client = getSupabaseClient();
  const cleanExt = (extension || "bin").replace(/^\./, "");
  const safePatient = String(patientId || "unknown").replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 80);
  const objectPath = `patient-journey-scans/${safePatient}/${Date.now()}-${randomUUID()}.${cleanExt}`;
