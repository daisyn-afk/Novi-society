...
export default function ProviderPractice() {
  ...
  useEffect(() => {
    ...
    if (["profile", "treatments", "appointments", "patients", "performance"].includes(tab)) {
      setActiveTab(tab);
    }
  }, [searchParams]);
  const [form, setForm] = useState({
    ...
    referral_program_active: false, referral_code: "", referral_discount: "",
    gallery_photos: [],
  });
  ...
  useEffect(() => {
    if (me && !initialized) {
      setForm({
        ...
        gallery_photos: me.gallery_photos || [],
      });
      setInitialized(true);
    }
  }, [me, initialized]);
  ...
  const documentedApptIds = new Set(treatmentRecords.map((r) => r.appointment_id).filter(Boolean));
  const needsDocumentation = appointments.filter(
    (a) => a.status === "completed" && !documentedApptIds.has(a.id)
  );
  ...
  const practiceContent = (
    <div className="max-w-5xl mx-auto">
      ...
      {pendingCount > 0 && (
        <div className="flex items-start gap-3 px-5 py-4 rounded-2xl mb-5" style={{ background: "rgba(254,226,226,0.85)", border: "1.5px solid rgba(220,38,38,0.45)" }}>
          <AlertTriangle className="w-5 h-5 mt-0.5 flex-shrink-0 text-red-600" />
          <div className="flex-1">
            <p className="font-semibold text-sm text-red-800">
              {pendingCount} patient{pendingCount > 1 ? "s are" : " is"} waiting for a response
            </p>
            <p className="text-xs mt-0.5 text-red-700/80">
              Confirm or decline appointment requests so patients are not left waiting.
            </p>
          </div>
          <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white" onClick={() => setActiveTab("appointments")}>
            Review requests
          </Button>
        </div>
      )}
      ...
      {pendingCount > 0 && (
        <div className="rounded-2xl px-5 py-4 mb-5" style={{ background: "linear-gradient(135deg, #1e2535 0%, #2a3550 100%)", border: "1px solid rgba(200,230,60,0.25)" }}>
          <p className="text-[10px] font-black uppercase tracking-widest mb-1" style={{ color: "rgba(200,230,60,0.8)" }}>Practice Hub focus</p>
          <h3 className="text-lg font-bold text-white" style={{ fontFamily: "'DM Serif Display', serif" }}>Don&apos;t Leave Them Waiting</h3>
          <p className="text-sm mt-1 text-white/60">
            Patients booked you expecting a quick reply. Open Appointments → Requests to confirm or decline.
          </p>
          <Button size="sm" className="mt-3 font-bold" style={{ background: "#C8E63C", color: "#1e2535" }} onClick={() => setActiveTab("appointments")}>
            Go to Requests
          </Button>
        </div>
      )}
      ...
      {needsDocumentation.length > 0 && (
        <div className="flex items-start gap-3 px-5 py-4 rounded-2xl mb-5" style={{ background: "rgba(123,142,200,0.12)", border: "1px solid rgba(123,142,200,0.35)" }}>
          <FileText className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: "#7B8EC8" }} />
          <div className="flex-1">
            <p className="font-semibold text-sm" style={{ color: "#1e2535" }}>
              Needs Documentation: {needsDocumentation.length} completed visit{needsDocumentation.length > 1 ? "s" : ""}
            </p>
            <p className="text-xs mt-0.5" style={{ color: "rgba(30,37,53,0.5)" }}>
              Your MD requires a treatment record for every completed session.
            </p>
          </div>
          <Button size="sm" variant="outline" onClick={() => setActiveTab("appointments")}>Document now</Button>
        </div>
      )}
