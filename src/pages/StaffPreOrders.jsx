...
function ApplicationRow({ order, onApprove, onReject, isProcessing }) {
  const [expanded, setExpanded] = useState(false);
  const [rejectionReason, setRejectionReason] = useState("");
  const [showRejectForm, setShowRejectForm] = useState(false);

  const isPending = order.status === "pending_approval";
  const canExpand = !PAID_STATUSES.has(order.status);
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs truncate" style={{ color: "rgba(30,37,53,0.55)" }}>
            ...
            {!PAID_STATUSES.has(order.status) && (
              <> · {order.order_type === "course" ? order.course_title : order.service_name}
        ...
        <div className="px-5 pb-5 border-t" style={{ borderColor: "rgba(30,37,53,0.08)" }}>
          ...
          {isPending && !showRejectForm && (
            <div className="flex gap-3">
              <Button onClick={() => onApprove(order.id)} disabled={isProcessing} className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold" style={{ background: "#C8E63C", color: "#1a2540", border: "none" }}>
                <CheckCircle2 className="w-4 h-4" /> Approve & Send Payment Link
              </Button>
              <Button onClick={() => setShowRejectForm(true)} variant="outline" className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold" style={{ border: "1px solid rgba(218,106,99,0.4)", color: "#DA6A63", background: "transparent" }}>
          ...
          {isPending && showRejectForm && (
            <div className="space-y-3">
              ...
              />
              <div className="flex gap-3">
                <Button onClick={() => onReject(order.id, rejectionReason)} disabled={isProcessing} className="px-5 py-2 rounded-xl text-sm font-bold" style={{ background: "#DA6A63", color: "#fff", border: "none" }}>
                  Confirm Rejection
                </Button>
                <Button onClick={() => setShowRejectForm(false)} variant="ghost" className="px-5 py-2 rounded-xl text-sm" style={{ color: "rgba(30,37,53,0.5)" }}>
                  Cancel
                </Button>
              </div>
            </div>
...
export default function StaffPreOrders() {
  ...
  const actionMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["staff-pre-orders"] }),
  });

  const courseOrders = preOrders.filter(o => o.order_type !== "model");

  const filtered = statusFilter === "all"
    ? courseOrders
    : statusFilter === "paid"
      ? courseOrders.filter(o => PAID_STATUSES.has(o.status))
      : courseOrders.filter(o => o.status === statusFilter);

  const pendingCount = courseOrders.filter(o => o.status === "pending_approval").length;

  return (
    <div className="space-y-6 max-w-5xl w-full">
      <div>
        <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, color: "#1e2535", lineHeight: 1.15 }}>Pre-Order Applications</h1>
        <p style={{ color: "rgba(30,37,53,0.58)", fontSize: 13, marginTop: 6 }}>
          Review and approve course enrollment applications
        </p>
      </div>

      <div className="grid grid-cols-4 gap-3">
        {[
          { label: "Pending Approval", value: courseOrders.filter(o => o.status === "pending_approval").length, color: "#FA6F30" },
          { label: "Awaiting Payment", value: courseOrders.filter(o => o.status === "payment_link_sent").length, color: "#7B8EC8" },
          { label: "Paid / Confirmed", value: courseOrders.filter(o => PAID_STATUSES.has(o.status)).length, color: "#a8cc20" },
          { label: "Total Applications", value: courseOrders.length, color: "rgba(30,37,53,0.7)" },
        ].map((s, i) => (
      ...
      <div className="flex gap-2 flex-wrap">
        {STATUS_TABS.map(tab => (
          ...
          >
            {tab.label}{tab.value === "pending_approval" && pendingCount > 0 ? ` (${pendingCount})` : ""}
          </button>
        ))}
      </div>
