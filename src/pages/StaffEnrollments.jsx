...
function EnrollmentRow({ enrollment, courseTitle }) {
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "rgba(30,37,53,0.7)" }}>
                {enrollment.session_date
                  ? new Date(enrollment.session_date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })
                  : "—"}
              </p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "rgba(30,37,53,0.4)" }}>Status</p>
              <StatusBadge status={enrollment.status} />
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "rgba(30,37,53,0.4)" }}>Enrolled</p>
              <p className="text-sm" style={{ color: "rgba(30,37,53,0.7)" }}>
                {enrollment.created_date ? format(new Date(enrollment.created_date), "MMM d, yyyy") : "—"}
              </p>
            </div>
...
export default function StaffEnrollments() {
  ...
  const filtered = useMemo(() => {
    ...
    return paidEnrollments.filter(e => {
      const courseTitle = courseMap[e.course_id]?.title || "";
      const byCourse = courseFilter === "all" || String(e.course_id) === String(courseFilter);
      const bySearch = !term
        || String(e.provider_name || "").toLowerCase().includes(term)
        || String(courseTitle).toLowerCase().includes(term);
  ...
  return (
    <div className="space-y-6 max-w-5xl w-full">
      ...
      <div className="relative">
        ...
        <Input className="pl-9" placeholder="Search by provider name or course..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
        ...
        <>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 rounded-2xl" style={{ background: "rgba(30,37,53,0.04)", border: "1px solid rgba(30,37,53,0.08)" }}>
              ...
              <p style={{ color: "rgba(30,37,53,0.4)" }}>No enrollments found</p>
            </div>
          ) : (
            <div>
              {filtered.map(e => (
