...
export default function AdminCourses() {
  ...
  const { data: allCourses = [], isLoading: loadingCourses } = useQuery({ queryKey: ["courses"], queryFn: () => base44.entities.Course.list("-created_date") });
  ...
  const saveTemplate = useMutation({
    mutationFn: (data) => editingTemplate ? base44.entities.Course.update(editingTemplate, data) : base44.entities.Course.create({ ...data, type: "template" }),
    onSuccess: () => { qc.invalidateQueries(["courses"]); setTemplateOpen(false); setTemplateForm(EMPTY_TEMPLATE); setEditingTemplate(null); },
  });
  ...
  const saveScheduled = useMutation({
    mutationFn: (data) => {
      ...
      return editingSchedule ? base44.entities.Course.update(editingSchedule, merged) : base44.entities.Course.create(merged);
    },
    onSuccess: () => { qc.invalidateQueries(["courses"]); setScheduleOpen(false); setScheduleForm(EMPTY_SCHEDULED); setEditingSchedule(null); },
  });
