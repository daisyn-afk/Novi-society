import { Clock, MapPin, Award, BookOpen, FileText, Zap } from "lucide-react";
...
export default function CourseBrowseCard({ course, isEnrolled, onSelect, onViewMaterials, enrollmentStatusLoading = false }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Pre-Course Materials — visible before purchase */}
        {course.pre_course_materials?.length > 0 && (
          ...
          >
            <p className="text-xs font-bold mb-2 flex items-center gap-1.5" style={{ color: "#7B8EC8" }}>
              <BookOpen className="w-3.5 h-3.5 flex-shrink-0" />
              Pre-Course Materials to Study
            </p>
              ...
              {course.pre_course_materials.map((mat, i) => (
                <li key={i} className="text-xs flex items-start gap-2" style={{ color: "rgba(30,37,53,0.65)" }}>
                  {mat.required !== false ? (
                    ...
                    <FileText className="w-3 h-3 flex-shrink-0 mt-0.5" style={{ color: "#7B8EC8" }} />
                  )}
                  <span>
                    {mat.title}
                    {mat.required !== false ? " (Required)" : ""}
                  </span>
                </li>
              ))}
            </ul>
            {onViewMaterials && (
              <button
                ...
                className="w-full mt-2.5 py-2 rounded-lg text-xs font-bold transition-all hover:opacity-90"
