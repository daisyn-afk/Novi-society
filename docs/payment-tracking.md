...
**Status:** Implemented -- one consolidated migration -- ready for develop merge + live-site apply
...
Two tables and one view in the `public` schema, all created by a single migration:

| Migration | What it creates |
|---|---|
| `20260515120000_payment_tracking.sql` | `payment_transactions` table (all columns), `payment_events` table (all columns), all indexes, `updated_at` trigger, immutability trigger + function, `payment_attempt_timeline` forensic view. |
...
| Flow classification | `payment_flow` (`course` / `model` / `service`), `payment_type`, `payment_status`, `previous_status` |
...
- B-tree indexes on `payment_status`, `payment_flow`, `customer_email`, `pre_order_id`, `stripe_payment_intent_id`, `stripe_charge_id`, `stripe_customer_id`, `created_at DESC`, `selected_item_id`, `(customer_email, payment_flow, attempt_number DESC)`.
...
| Free-form | `metadata` (includes `status_transition` delta for webhook events), `raw_event` (full Stripe object) |
...
supabase/migrations/20260515120000_payment_tracking.sql   # Single DB migration (all tables, triggers, view)
```
...
|                                                                     |
|  1. recordCheckoutInitiated(...)  -> payment_transactions (initiated)|
|                                      payment_events (initiated)     |
|  2. Validates course, seats, promo code (DB, row-locked)            |
|  3. Inserts pre_orders row        -> status = pending_payment       |
|  4. Calls stripe.checkout.sessions.create(...)                      |
|       metadata + payment_intent_data.metadata both set              |
|  5. Updates pre_orders with stripe_session_id                       |
|  6. enrichPaymentTransaction({id})-> attaches stripe_session_id,    |
|                                      pre_order_id, amounts          |
|  Returns: { checkout_url, stripe_session_id, pre_order_id }         |
|  [On any error: markAttemptFailed -> status = failed]               |
+-----------------------------+---------------------------------------+
...
|  | .expired webhook   |    | .payment_failed webhook|              |
...
|  1. recordCheckoutInitiated(...)  -> payment_transactions (initiated)|
|                                      payment_events (initiated)    |
|  2. Validates course date, time slot, seat cap                     |
|  3. Applies promo code (if any)                                    |
|  4. Inserts pre_orders row (order_type = model)                    |
...
|  6. enrichPaymentTransaction({id}) -> stripe_session_id attached   |
...
|                                                                     |
|  1. recordPreOrderInitiated(...)  -> payment_transactions          |
...
|  2. Validates service_type_id                                      |
|  3. Inserts pre_orders row                                         |
...
- Re-delivering the same event = 0 net change.
...
| Scenario | Behaviour |
|---|---|
| User enters wrong / invalid `course_id` and clicks Pay | A `payment_transactions` row is created with the exact `request_payload_snapshot`, `selected_item_id` set to the literal user input, and `payment_status = 'failed'` plus a populated `failure_message`. |
...
| User double-clicks Pay | Each click creates a new Stripe session ID and therefore a new row. |
...
Implementation invariants (enforced by code and DB trigger):

1. `recordCheckoutInitiated` performs a plain `INSERT` (no `ON CONFLICT DO UPDATE`). Every call yields a new primary key.
2. `request_payload_snapshot` is stored as `jsonb`, deep-copying the request body verbatim. This is the **source of truth** for "what did the user submit".
3. `selected_item_id` is stored uncoerced (`text`). If the user typed `"this-is-not-a-uuid"`, that exact string is in the column.
4. `attempt_number` is computed at insert time via `coalesce((select max(attempt_number) + 1 ...), 1)`.
5. `client_timestamp` is captured by the frontend at the click moment. `server_received_timestamp` is set on arrival. A large gap (>30s) suggests the browser tab was idle or using stale state.
6. After Stripe session creation succeeds, the route calls `enrichPaymentTransaction({ id: attemptId }, { stripe_session_id, ... })` using `coalesce` so once a value is set it is never silently overwritten.
...
The entire schema is in **one idempotent file**. Every statement uses `CREATE ... IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS` / `CREATE OR REPLACE`, so the file is safe to re-run.
...
Or apply through Supabase's migration dashboard / your team's existing pipeline.
...
In the Stripe Dashboard -> **Developers -> Webhooks -> Add endpoint**, register **one endpoint** per environment:

| Purpose | Endpoint URL |
|---|---|
| All checkout flows (course, model, MD service) | `https://<your-host>/api/webhooks/stripe` |

The main endpoint routes model events internally via the `checkout_type` metadata field. The legacy `/api/functions/modelCheckoutWebhook` endpoint still works as a fallback but does not need to be registered separately.
...
If you ever see the log line `[webhook/stripe] rejected: req.body is not a Buffer (got object)`, this config was lost or overridden.
...
4. **Read the server logs.** All webhook entry points log:
...
| `charge.captured` | `charge_captured` | `succeeded` | Manual-capture flow (recorded if it appears). |
...
Both webhook endpoints (`POST /api/webhooks/stripe` and `POST /api/functions/modelCheckoutWebhook`) call `recordStripeWebhookEvent` for every event in this table.
...
The trigger `trg_payment_transactions_immutability` (defined in `20260515120000_payment_tracking.sql`) raises an exception on any UPDATE that violates the rules. The exception bubbles up to the caller -- there is no silent overwrite.
...
| Raw Stripe event object | `payment_events.raw_event` (sanitized; no PAN/CVV -- Stripe doesn't send them anyway) |
...
|---|---|---|
| `supabase/migrations/20260515120000_payment_tracking.sql` | **new** | Single consolidated migration: creates `payment_transactions` (all columns incl. snapshot + immutability fields) + `payment_events`, all indexes, `updated_at` trigger, immutability trigger + function, `payment_attempt_timeline` view. |
